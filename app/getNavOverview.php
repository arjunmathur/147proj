<?php
	...
	$query2 = "SELECT * FROM trip_participants WHERE trip_id = ".$tripId." ORDER BY time_to_dest_value_seconds DESC";
	...
	while($row2){
		...
		if($thisUser == $_POST["username"]){
			...
			$listData = $listData."<li><FONT COLOR='38C5FC'><h2>$thisUser</h2></FONT><p><strong> $dist / $time</strong></p></li>";
		} else {
			if($status == 0){
				...
				$listData = $listData."<li><h3>$thisUser</h3><p><strong>$dist / $time</strong></p><p>Arrived</p></li>";
			}else{
